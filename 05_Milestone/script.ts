function generateUniqueLink() {
  const name =
    (document.getElementById("name") as HTMLElement)?.innerText || "User";
  const baseUrl = window.location.origin;
  const formattedName = name.replace(/\s+/g, "-").toLowerCase();
  return `${baseUrl}/resume/${formattedName}`;
}
...
function editSection(section: string): void {
  ...
  switch (section) {
    case "personal":
      ...
      if (inputField) {
        inputField.value = (
          document.getElementById("job") as HTMLElement
        ).innerText;
        inputField.focus();
      }
    ...
    case "summary":
      ...
      if (inputField) {
        inputField.value = (
          document.getElementById("summary") as HTMLElement
        ).innerText;
        inputField.focus();
      }
      break;
    ...
    case "skills":
      ...
      if (inputField) {
        const skills = Array.from(
          document.querySelectorAll("#skills-content li")
        )
    ...
    case "education":
      ...
      if (inputField) {
        inputField.value = (
          document.getElementById("education") as HTMLElement
        ).innerText;
        inputField.focus();
      }
      break;
    ...
    case "experience":
      ...
      if (inputField) {
        const experiences = Array.from(
          document.querySelectorAll("#experience-content p")
        )
...
const summaryDisplay = document.getElementById(
  "summary-content"
) as HTMLElement;
...
document.getElementById("summary-form")?.addEventListener("submit", (event) => {
  ...
  summaryDisplay.textContent =
    summaryInput.value ||
    "A highly motivated web developer with experience in creating dynamic and responsive web applications.";
  summaryInput.value = "";
});
